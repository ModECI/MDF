...
ABCD.add_projection(projection=pnl.MappingProjection(name='MappingProjection from A[RESULT] to B[InputPort-0]', function=pnl.LinearMatrix(matrix=[[1.0]], default_variable=[2.0])), sender=A, receiver=B)
ABCD.add_projection(projection=pnl.MappingProjection(name='MappingProjection from A[RESULT] to C[InputPort-0]', function=pnl.LinearMatrix(matrix=[[1.0]], default_variable=[2.0])), sender=A, receiver=C)
ABCD.add_projection(projection=pnl.MappingProjection(name='MappingProjection from B[RESULT] to D[InputPort-0]', function=pnl.LinearMatrix(matrix=[[1.0]], default_variable=[0.5])), sender=B, receiver=D)
ABCD.add_projection(projection=pnl.MappingProjection(name='MappingProjection from C[RESULT] to D[InputPort-0]', function=pnl.LinearMatrix(matrix=[[1.0]], default_variable=[1.0])), sender=C, receiver=D)
