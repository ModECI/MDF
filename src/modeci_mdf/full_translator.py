            ...
            if "parameters" in d[key].keys():
                ...
                for idx, param in enumerate(list(d[key]["parameters"].keys())):
                    if "time_derivative" in d[key]["parameters"][param].keys():
                        ...
                        d[key]["functions"][
                            "evaluated_{}_{}_next_value".format(key, param)
                        ] = {}
                        d[key]["functions"][
                            "evaluated_{}_{}_next_value".format(key, param)
                        ]["value"] = {}
                        ...
                        d[key]["functions"][param]["function"] = d[key]["parameters"][
                            param
                        ]["function"]
                        d[key]["functions"][param]["args"] = {}
                        d[key]["functions"][param]["args"] = d[key]["parameters"][
                            param
                        ]["args"]
                    ...
                    if idx > 0:
                        for prev_param in parameterlist[:-1]:
                            ...
                            d[key]["functions"][
                                "evaluated_{}_{}_next_value".format(key, param)
                            ]["value"] = d[key]["functions"][
                                "evaluated_{}_{}_next_value".format(key, param)
                            ][
                                "value"
                            ].replace(
                                prev_param,
                                "evaluated_{}_{}_next_value".format(key, prev_param),
                            )
                ...
                for idx, output_port in enumerate(list(d[key]["output_ports"].keys())):
                    if isinstance(d[key]["output_ports"][output_port]["value"], str):
                        if any(
                            ...
                            d[key]["functions"][
                                "evaluated_{}_{}_value".format(key, output_port)
                            ] = {}
                            d[key]["functions"][
                                "evaluated_{}_{}_value".format(key, output_port)
                            ]["value"] = {}
                            d[key]["functions"][
                                "evaluated_{}_{}_value".format(key, output_port)
                            ]["value"] = expression_dict[key][output_port]
            ...
            if "parameters" in d[key].keys():
                for param in list(d[key]["parameters"].keys()):
                    if "time_derivative" in d[key]["parameters"][param].keys():
                        ...
                        d[key]["functions"]["evaluated_time_next_value"]["args"] = {
                            "variable0": "time",
                            "slope": 1,
                            "intercept": "dt",
                        }
