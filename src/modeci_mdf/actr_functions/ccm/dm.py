...
class Memory(Model):
    ...
    def request(self, pattern, partial=None, require_new=False):
        if partial is None and len(self.partials) > 0:
        ...
        all = self.dm
        if require_new:
            all = [x for x in all if not self.finst.contains(x)]
    ...
    def fail(self, request_number):
        if self.threshold is None:
            ...
            if time > self.maximum_time:
                time = self.maximum_time
        yield time
        if request_number != self._request_count:
            return
...
class MemorySubModule:
    ...
    def now(self):
        if self.parent is None or not self.parent._is_converted():
            return 0
        else:
    ...
    def merge(self, chunk, time=0.0, baselevel=None, **keys):
        ...
        if self.limit is not None and len(chunk.times) > self.limit:
            if self.limit == 0:
                ...
                chunk.times = chunk.times[-self.limit :]
        if baselevel is not None:
            if baselevel == "calculate":
                del chunk.baselevel
            else:
                chunk.baselevel = baselevel
        ...
        if self.limit is not None:
            ...
            if n > k:
                ...
                approx = (
                    (n - k)
                    / (1 - d)
                    * (math.pow(tn, 1 - d) - math.pow(tk, 1 - d))
                    / (tn - tk)
                )
...
class DMSalience(MemorySubModule):
    ...
    def context(self, pattern):
        ...
        for k, hist in self.histogram.items():
            hist.clear()
        if len(chunks) == 0:
            raise Exception('No chunks match salience pattern: "%s"' % pattern)
        dw = 1.0 / len(chunks)
        for c in chunks:
            for k, hist in self.histogram.items():
                val = c.get(k, None)
