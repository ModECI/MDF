...
class A(nn.Module):
    ...
    def forward(self, variable0):
        self.execution_count = self.execution_count + torch.tensor(1)
        linear_func = variable0 * self.slope + self.intercept
        return linear_func
...
class B(nn.Module):
    ...
    def forward(self, variable0):
        self.execution_count = self.execution_count + torch.tensor(1)
        logistic_func = 1 / (
            1 + exp(-1 * self.gain * (variable0 + self.bias) + self.offset)
        )
...
class C(nn.Module):
    ...
    def forward(self, variable0):
        self.execution_count = self.execution_count + torch.tensor(1)
        exponential_func = (
            self.scale * exp((self.rate * variable0) + self.bias) + self.offset
        )
...
class D(nn.Module):
    ...
    def forward(self, variable0):
        self.execution_count = self.execution_count + torch.tensor(1)
        sin_func = self.scale * sin(variable0)
        return sin_func
